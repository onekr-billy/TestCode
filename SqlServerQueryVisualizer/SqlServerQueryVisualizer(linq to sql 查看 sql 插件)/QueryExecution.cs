...
namespace SqlServerQueryVisualizer {
    ...
    public class DialogChooser : DialogDebuggerVisualizer {
        ...
        public static void TestShowVisualizer(object objectToVisualize) {
            VisualizerDevelopmentHost visualizerHost = new VisualizerDevelopmentHost(objectToVisualize, typeof(DialogChooser), typeof(SourceChooser));
    ...
    internal static class Utils {
        ...
        internal static Type GetNonNullableType(Type clrType) {
                ...
                switch (param.SqlType.ToString()) {
                    ...
                    case "Boolean":
                        if (param.Value == "True") {
                            ...
                            throw new Exception("Boolean value other than True or False");
                        }
                        break;
                    case "Time":
    ...
    internal static class QueryExecution {
        ...
        private static SqlDbType GetSqlDbTypeFromText(string text) {
            // if the type is "decimal(29,8)", only take "decimal"
            int pos = text.IndexOf('(');
            if (pos >= 0) {
                text = text.Substring(0, pos);
            }
            return (SqlDbType)Enum.Parse(typeof(DbType), text.Trim());
        }
        ...
        private static object GetObject(string val, string sqlType) {
            ...
            //if (Utils.IsNullableType(type)) {
            //    if (val == "") {
            //        return DBNull.Value;
            //    }
            //}
            //Type nnType = Utils.GetNonNullableType(type);
            ...
            if (nnType == DbType.String) {
                ...
                throw new Exception("Type " + sqlType + " is not supported for parameters in Sql Server Query Visualizer");
            }
        }
        ...
        private static SqlCommand GetSqlCommand(SqlQueryText qt, SqlConnection conn) {
            ...
            foreach (ParameterText param in qt.Params) {
                ...
                Console.WriteLine("Param.name:" + param.Name + "Param.SqlType:" + param.SqlType);
